...
const MIGRATIONS = [
    ...
    },
    {
        ...
        name: 'Create simplified roles system with 2 fixed roles',
        async execute(client) {
            console.log('🔄 Ejecutando migración 037: Creando sistema de roles simplificado...');

            try {
                // Step 1: Create roles table (simplified - no complex permissions)
                ...
                // Step 2: Create system roles for all existing tenants
                console.log('   📝 Creando 2 roles fijos: "Acceso Total" y "Acceso Repartidor"...');

                // Get all tenants
                const tenantsResult = await client.query('SELECT id FROM tenants');

                for (const tenant of tenantsResult.rows) {
                    // Check if "Acceso Total" role exists
                    const accesoTotalCheck = await client.query(
                        'SELECT id FROM roles WHERE tenant_id = $1 AND name = $2',
                        [tenant.id, 'Acceso Total']
                    );

                    if (accesoTotalCheck.rows.length === 0) {
                        ...
                        console.log(`   ✅ Rol "Acceso Total" creado para tenant ${tenant.id}`);
                    }

                    // Check if "Acceso Repartidor" role exists
                    const accesoRepartidorCheck = await client.query(
                        'SELECT id FROM roles WHERE tenant_id = $1 AND name = $2',
                        [tenant.id, 'Acceso Repartidor']
                    );

                    if (accesoRepartidorCheck.rows.length === 0) {
                        ...
                        console.log(`   ✅ Rol "Acceso Repartidor" creado para tenant ${tenant.id}`);
                    }
                }
                console.log('   ✅ Ambos roles creados para todos los tenants');

                console.log('✅ Migración 037 completada: Sistema de roles simplificado creado');
            } catch (error) {
    ...
    },
    {
        ...
        name: 'Fix roles table to be global with fixed IDs matching Desktop (1=Admin, 2=Encargado, 3=Repartidor, 4=Ayudante)',
        async execute(client) {
            ...
            try {
                ...
                if (checkColumns.rows.length > 0) {
                    ...
                    // Just ensure the 4 fixed roles exist
